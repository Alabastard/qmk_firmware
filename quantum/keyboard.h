...
typedef enum keyevent_type_t { TICK_EVENT = 0, KEY_EVENT = 1, ENCODER_CW_EVENT = 2, ENCODER_CCW_EVENT = 3, COMBO_EVENT = 4, DIP_SWITCH_ON_EVENT = 5, DIP_SWITCH_OFF_EVENT = 6, POINTING_MODE_EVENT = 7, POINTING_VIRTKEY_EVENT = 8 } keyevent_type_t;
...
#define KEYLOC_POINTING_VIRTKEY 248
...
static inline bool IS_POINTINVIRTKEYGEVENT(const keyevent_t event) {
    return event.type == POINTING_VIRTKEY_EVENT;
}
...
#    define MAKE_POINTING_VIRTKEY_EVENT(switch_id, press) MAKE_EVENT(KEYLOC_POINTING_VIRTKEY, (switch_id), (press), POINTING_VIRTKEY_EVENT)
#endif

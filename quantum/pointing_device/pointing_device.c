...
#    ifdef POINTING_VIRTKEY_MAP_ENABLE
    if (pointing_device_driver.get_virtual_key_state) {
        pointing_device_modes_keys_task(pointing_device_driver.get_virtual_key_state());
    }
#    endif
...
#        if POINTING_MODE_SINGLE_CONTROL
    ...
    switch (pointing_mode_get_current_device()) {
...
#        else
    ...
    uint8_t current_device = pointing_mode_get_current_device();
    pointing_mode_set_current_device(PM_RIGHT_DEVICE);
    right_report = pointing_device_modes_task(right_report);

    pointing_mode_set_current_device(PM_LEFT_DEVICE);
    left_report = pointing_device_modes_task(left_report);
    // set device id back
    pointing_mode_set_current_device(current_device);
#        endif
#    endif
